...
namespace MES_APEM_UFT_Selenium_Auto.TestCase
{
    ...
    {
        [TestCaseID(1021810)]
        [Title("UC967898_The phase will change to Cancelled(Set CANCEL_PENDDING_WORKSTATION_OPER=1) when end user session function should work on Session Manager if user have administrator role in AFW security Manager.")]
        ...
        [TestMethod]
        public void VSTS_1021810()
        {
            string Resultpath = Base_Directory.ResultsDir + CaseID + "-";
            string OrderName1 = "Order1021810_1";
            string OrderName2 = "Order1021810_2";
            ...
            try
            {
                ...
                Mobile_Fuction.TakeScreenshot(Selenium_Driver._Selenium_Driver, Resultpath + "Phase is Cancelled.PNG");
                Base_Assert.IsTrue(edge.GetUrl().Contains("tracking"), "in tracking page");
                Base_Assert.IsTrue(Mobile.SessionManager_Page.TableRows[0].FindElement(By.TagName("mat-icon")).GetAttribute("data-mat-icon-name").Contains("phase_state_cancelled"), "Phase is Cancelled");
                ...
                Mobile_Fuction.login(UserName.qaone3, PassWord.qaone3);
                //switched to PFC page/order tracking page
                Mobile_Fuction.TakeScreenshot(Selenium_Driver._Selenium_Driver, Resultpath + "Phase is Cancelled dark mode.PNG");
                Base_Assert.IsTrue(edge.GetUrl().Contains("tracking"), "in tracking page");
                Base_Assert.IsTrue(Mobile.SessionManager_Page.TableRows[0].FindElement(By.TagName("mat-icon")).GetAttribute("data-mat-icon-name").Contains("phase_state_cancelled"), "Phase is Cancelled");
}
            finally
